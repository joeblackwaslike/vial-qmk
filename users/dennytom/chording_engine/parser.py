...
def parse_chords(data):
    ...
    keyboard_part_2 += (
        ...
        + reduce(
            comma_separator,
            ["&chord_" + str(i) for i in range(number_of_chords)],
        )
        + "\n"
    )
...
def parse_strings_for_chords(data):
    keyboard_part_1 = "".join(
        "const char string_" + str(i) + " [] PROGMEM = \"" + string + "\";\n"
    ...
    if len(strings) > 0:
        keyboard_part_1 += "    " + reduce(
            comma_separator, ["string_" + str(i) for i in range(len(strings))]
        )
